 ...
 * Next you need to load the required parameters file. In Mission Planner's full parameter list, click the _Load From File_ button.  Select the `.param` file.  Numerous boxes in the parameter list will turn green.  Click the _Write Params_ button to load the new parameters.
 *Power cycle the solo again. Everything will reconnect.
